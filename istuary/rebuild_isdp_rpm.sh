...
function rebuild_rpm()
{
    ...
    sh /root/difference-operation.sh $rpm_name ~/rpmbuild/SOURCES/ 1.5.0.0-001 ~/rpmbuild/SPECS/
    pushd ./ && cd ~/rpmbuild/SOURCES/

    # 重命名 hdp->isdp, 2.5.0.0->1.5.0.0, 1245->001, 目录, 普通文件和软连接分开操作
    # 目录, 文件名称, 软连接名称本身含有版本信息, rename 只替换首次匹配 
    find ./ -type d -name "hdp" | xargs rename hdp isdp
    find ./ -type d -name "2.5.0.0-1245" | xargs rename 2.5.0.0-1245 1.5.0.0-001
    # hdp 会在部分组建的字母下, 使用带 hdp 的前缀, 如 spark 组建下的 hdplib
    find ./ -type d -name "hdpLib" | xargs rename hdp isdp 
    find ./ -type d | xargs rename 2.5.0.0-1245 1.5.0.0-001
    find ./ -type d | xargs rename 2.5.0.0 1.5.0.0
    # find ./ -type f | xargs rename hdp isdp
    find ./ -type f | xargs rename 2.5.0.0-1245 1.5.0.0-001
    find ./ -type f | xargs rename 2.5.0.0-1245 1.5.0.0-001
    find ./ -type l | xargs rename 2.5.0.0-1245 1.5.0.0-001
    find ./ -type l | xargs rename 2.5.0.0-1245 1.5.0.0-001
    find ./ -type l | xargs rename -s hdp isdp
    find ./ -type l | xargs rename -s 2.5.0.0-1245 1.5.0.0-001
    find ./ -type l | xargs rename -s 2.5.0.0-1245 1.5.0.0-001
    find ./ -type l | xargs rename -s 2.5.0.0-1245 1.5.0.0-001
    find ./ -type l | xargs rename -s 2.5.0.0-1245 1.5.0.0-001
    ...
    do
        ...
        if [ -n "$is_text_file" ]; then
            sed -i 's/\/usr\/hdp/\/usr\/isdp/g' $text_file
            sed -i 's/2.5.0.0-1245/1.5.0.0-001/g' $text_file
        fi
    done
    
    # 重新打包源文件
    rpm_name=`echo $rpm_name | sed 's/2_5_0_0_1245/1_5_0_0_001/g'`
    rpm_name=`echo $rpm_name | sed 's/2.5.0.0-1245/1.5.0.0-001/g'`
    ...
    sed -i 's/usr\/hdp/usr\/isdp/g' ./${spec_file_name}
    sed -i 's/hdpLib/isdpLib/g' ./${spec_file_name}
    sed -i 's/_2_5_0_0_1245/_1_5_0_0_001/g' ./${spec_file_name}
    sed -i 's/2\.5\.0\.0-1245/1.5.0.0-001/g' ./${spec_file_name}
    sed -i 's/2\.5\.0\.0/1.5.0.0/g' ./${spec_file_name}
...
function rebuild_isdp_version_rpm()
{
    local spec_file_dir=$1
    local hdp_stack_dir=$2

    rm -rf isdp-rpm && mkdir -p isdp-rpm
...
# main function 
if [ $# -ne 2 ]; then
    echo "Usage : ./rebuild_isdp_version_rpm.sh spec_file_dir hdp_stack_dir"
    exit 1
fi 

# call rebuild function
rebuild_isdp_version_rpm $@
